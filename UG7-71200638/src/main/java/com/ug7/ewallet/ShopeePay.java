...
public class ShopeePay extends eWallet {
    ...
    public void topup(int jumlah) {
        if(jumlah + feeTransfer > super.getUser().getUang()) {
            jumlah += feeTransfer;
            System.out.println("Uang cash kamu tidak mencukupi! ("+formatRupiah(this.getSaldo()-jumlah)+")");
            return;
        }
        super.topup(jumlah);
    ...
    public void transfer(eWallet eWallet, int jumlah) {
        ...
        if(jumlah + feeTransfer > super.getSaldo()) {
            jumlah += feeTransfer;
            System.out.println("Saldo kamu tidak cukup! ("+formatRupiah(this.getSaldo()-jumlah)+")");
            return;
        }
